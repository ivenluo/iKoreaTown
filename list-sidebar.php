<aside class="shop-sidebar">
	...
	<?php 
		...
		if ( ! empty( $terms ) ) {
		    echo '<ul><li><a href="' . esc_url( get_term_link( $taxonomy_object ) ) . '">' . $taxonomy_name . '</a></li>';
		    echo 
	...
	<section class="widget">
	    ...
	    if ( ! empty( $terms ) ) {
		    echo '<ul>';
		    echo '<li><a href="' . esc_url( $current_url ) . '" class="' . $class . '">' . '전부' . '</a></li>';
		    foreach ( $terms as $term ) {
				...
				echo '<li><a href="' . esc_url( $current_url ) . '" class="' . $class . '">' . $term->name . '</a></li>';
		    }
		    echo '</ul>';
		}?>
	</section>
