...
/**
 * Set custom default avatar for buddypress
 *
 * @since 0.0.0
 */

add_filter('bp_core_fetch_avatar', 'kt_bp_remove_gravatar', 1, 9 );
function kt_bp_remove_gravatar ( $image, $params, $item_id, $avatar_dir, $css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir ) {
    $default = get_stylesheet_directory_uri() .'/img/default.jpg';
...
add_filter('get_avatar', 'kt_remove_gravatar', 1, 5);
function kt_remove_gravatar ( $avatar, $id_or_email, $size, $default, $alt ) {
    $default = get_stylesheet_directory_uri() .'/img/default.jpg';
    return "<img alt='{$alt}' src='{$default}' class='avatar avatar-{$size} photo avatar-default' height='{$size}' width='{$size}' />";
}

add_filter('bp_get_signup_avatar', 'kt_bp_remove_signup_gravatar', 1, 1 );
function kt_bp_remove_signup_gravatar ( $image ) {
    $default = get_stylesheet_directory_uri() .'/img/default.jpg';
    if( $image && strpos( $image, "gravatar.com" ) ){
